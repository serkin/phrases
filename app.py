...
def create_app():
    ...
    def answer():
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            """SELECT base, th, spelling FROM words WHERE id=%s LIMIT 1""", (word_id,))
        rv = cur.fetchall()
        ...
        cur.close()
        return render_template("answer.html", stat=_stat(), result=result, spelling=rv[0]["spelling"], question=rv[0]["base"], answer=answer, word_id=word_id, correct_answer=rv[0]["th"])
